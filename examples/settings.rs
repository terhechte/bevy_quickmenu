...
impl GameState {
    fn is_game(state: Res<State<GameState>>) -> ShouldRun {
        (state.current() == &GameState::Game).into()
    }

    fn is_settings(state: Res<State<GameState>>) -> ShouldRun {
        (state.current() == &GameState::Settings).into()
    }
}
    ...
    impl Plugin for SettingsPlugin {
        fn build(&self, app: &mut App) {
            app
                ...
                .add_system_set(
                    SystemSet::new()
                        .with_run_criteria(GameState::is_settings)
...
mod game {
    ...
    impl Plugin for Game {
        fn build(&self, app: &mut App) {
            app.add_system_set(SystemSet::on_enter(GameState::Game).with_system(setup_system))
                .add_system(detect_close_system.with_run_criteria(GameState::is_game))
                .run();
        }
    }
